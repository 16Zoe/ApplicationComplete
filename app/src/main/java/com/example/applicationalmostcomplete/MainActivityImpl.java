...
public class MainActivityImpl  extends RobotActivity implements RobotLifecycleCallbacks {
    ...
    private void textViewSetValue(String val) {
        runOnUiThread(() -> textView.setText(val));
    }

    private void imageViewSetValue(String val) {
        if (val.equals("juice"))
            runOnUiThread(() -> imageView.setImageResource(R.drawable.juice));
    ...
    public void initActions() {
        ...
        Animation animation_bad = AnimationBuilder.with(qiContext) // Create the builder with the context.
                .withResources(R.raw.sad_a003) // Set the animation resource.
                .build(); // Build the animation.
